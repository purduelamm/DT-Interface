...
class CRX10IALEnv(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        self._arm = Arm(
            xml_path= os.path.join(
                ...
                '../assets/robots/fanuc_robots_crx10ial/crx10ial.xml',
            ),
        ...
        # 2f85 gripper
        self._gripper = robotiq_2f85.RQ_2f85()

        # attach gripper to arm
        self._arm.attach_tool(self._gripper.mjcf_model)

        self._core_maker = mjcf.from_path(os.path.join(
                os.path.dirname(__file__),
                '../assets/machines/VMC-300-l v2/VMC-300-l v2.xml')
            )

        # attach arm to arena
        self._arena.attach(
            self._arm.mjcf_model, pos=[0,0,0.73], quat=[0.7071068, 0, 0, -0.7071068]
        )

        self._arena.attach(
            self._core_maker, pos=[1.5,-1.8,0.73], quat=[0.5, -0.5, -0.5, 0.5]
        )
    ...
    def step(self, action: np.ndarray) -> tuple:
        ...
        self._physics.data.ctrl[:] = action
